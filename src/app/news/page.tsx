  ...
  return (
    <div className="min-h-screen gradient-bg">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-12 content-animate">
          ...
          <div className="space-y-6">
            {internalNewsData.map((item, index) => (
              <div key={item.id} className={`${isSalam ? 'bg-white border border-[#003931]' : 'bg-slate-900 border border-slate-700'} hover:${isSalam ? 'bg-[#EEFDEC]' : 'bg-slate-800'} group p-6 flex flex-col md:flex-row items-start md:items-center gap-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 stagger-animate`} style={{ animationDelay: `${0.1 * (index + 1)}s` }}>
                <div className={`flex-shrink-0 w-16 h-16 ${isSalam ? 'bg-[#EEFDEC]' : 'bg-slate-800'} hover:${isSalam ? 'bg-[#36C639]' : 'bg-slate-700'} rounded-lg flex items-center justify-center group-hover:scale-110 transition-all duration-300`}>
                  <NewspaperIcon className={`w-8 h-8 ${isSalam ? 'text-[#36C639]' : 'text-green-400'}`} />
                </div>
                <div className="flex-1">
                  <h3 className={`text-xl font-bold ${isSalam ? 'text-[#003931]' : 'text-white'} mb-2`}>{item.title[lang]}</h3>
                  <p className={`${isSalam ? 'text-[#005147]' : 'text-slate-400'} mb-2`}>{item.summary[lang]}</p>
                  <div className={`flex items-center gap-2 ${isSalam ? 'text-[#005147]' : 'text-slate-500'} text-sm`}>
        ...
        <div className="mb-12 content-animate">
          <div className="flex items-center gap-3 mb-6">
            <GlobeAltIcon className={`w-6 h-6 ${isSalam ? 'text-[#36C639]' : 'text-blue-400'}`} />
            <h2 className={`${isSalam ? 'section-title' : 'heading-2'}`}>
          ...
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className={`text-lg font-semibold ${isSalam ? 'text-[#003931]' : 'text-white'}`}>
        ...
        <div className="mt-16 flex justify-center content-animate">
          <div className={`${isSalam ? 'bg-white border border-[#003931]' : 'bg-slate-900 border border-slate-700'} p-8 rounded-2xl flex flex-col items-center w-full max-w-2xl shadow-2xl`}>
            <h3 className={`text-2xl md:text-3xl font-bold mb-2 text-center ${isSalam ? 'text-[#003931]' : 'text-white'}`}>
              {lang === 'ar' ? 'تابعنا على منصاتنا الاجتماعية' : 'Follow Us on Social Media'}
            </h3>
            <p className={`${isSalam ? 'text-[#005147]' : 'text-slate-400'} mb-8 text-center text-lg max-w-xl`}>
              {lang === 'ar' 
            ...
            <div className="flex flex-col md:flex-row gap-4 md:gap-8 w-full justify-center items-center">
              ...
              <button
                ...
                className={`group flex items-center gap-3 w-full md:w-auto justify-center px-5 py-4 rounded-2xl border-2 ${isSalam ? 'border-[#003931] bg-[#003931e6] hover:border-[#00F000] hover:shadow-[#00F000]/30' : 'border-[#0077b5] bg-[#0077b5]/90 hover:border-green-500 hover:shadow-green-500/30'} shadow-lg min-w-[140px] transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-400 hover:-translate-y-1`}
                style={{ boxShadow: '0 4px 24px 0 rgba(16,185,129,0.10)' }}
              >
